...
template <typename T>
void MusaBitwiseAndKernelLauncher(const T* input_a, const T* input_b, T* output,
                                  int64_t size);

template <typename T>
void MusaBitwiseAndBroadcastKernelLauncher(const T* input_a, const T* input_b,
                                           T* output,
                                           const int64_t* output_shape,
                                           const int64_t* a_strides,
                                           const int64_t* b_strides, int ndims,
                                           int64_t size);
 ...
 public:
  ...
  void Compute(OpKernelContext* ctx) override {
    ...
    OP_REQUIRES(ctx, bcast.IsValid(),
                errors::InvalidArgument("Incompatible shapes for BitwiseAnd: ",
    ...
    if (input_a.shape() == input_b.shape()) {
      MusaBitwiseAndKernelLauncher<T>(input_a.flat<T>().data(),
                                      input_b.flat<T>().data(),
                                      output->flat<T>().data(), size);
    } else {
      const auto& out_shape_vec = bcast.output_shape();
      const int ndims = out_shape_vec.size();

      OP_REQUIRES(
          ctx, ndims <= kMaxBroadcastDims,
          errors::InvalidArgument("BitwiseAnd broadcast supports at most ",
      ...
      musaMemcpyAsync(d_output_shape, h_output_shape, ndims * sizeof(int64_t),
                      musaMemcpyHostToDevice, stream);
      musaMemcpyAsync(d_a_strides, h_a_strides, ndims * sizeof(int64_t),
                      musaMemcpyHostToDevice, stream);
      musaMemcpyAsync(d_b_strides, h_b_strides, ndims * sizeof(int64_t),
                      musaMemcpyHostToDevice, stream);

      MusaBitwiseAndBroadcastKernelLauncher<T>(
          input_a.flat<T>().data(), input_b.flat<T>().data(),
          output->flat<T>().data(), d_output_shape, d_a_strides, d_b_strides,
          ndims, size);
    }
