...
template <typename T>
class MusaMinOp : public MusaOpKernel {
 public:
  explicit MusaMinOp(OpKernelConstruction* ctx) : MusaOpKernel(ctx) {
    OP_REQUIRES_OK(ctx, ctx->GetAttr("keep_dims", &keep_dims_));
  }

  // Min (reduction) is computationally intensive
  ...
  void Compute(OpKernelContext* ctx) override {
    ...
    ::musa::dnn::Reduce op;
    MTOP_CHECK_OK(op.SetMode(::musa::dnn::Reduce::Mode::MIN), "Set Reduce MIN",
                  ctx);
    MTOP_CHECK_OK(
    ...
    OP_REQUIRES(
        ...
        errors::Internal("MUSA muDNN Reduce Min execution failed. Status: ",
...
REGISTER_MUSA_MIN(Eigen::half);
REGISTER_MUSA_MIN(bfloat16);
