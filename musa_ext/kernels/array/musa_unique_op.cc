 ...
 public:
  ...
  void Compute(OpKernelContext* ctx) override {
    ...
    if (QueryMusaKernelRuntimeView(ctx).mudnn_handle == nullptr) {
      ctx->SetStatus(MusaMudnnHandleRequiredError());
      return;
    }
    ...
    OP_REQUIRES(ctx, stream != nullptr,
                errors::Internal("MUSA stream is unavailable for Unique"));
    std::vector<int64_t> nd_info;
    t_counts.GetNdInfo(nd_info);
    int64_t num_unique = nd_info[0];
    Tensor* out_values = nullptr;
    OP_REQUIRES_OK(
        ctx, ctx->allocate_output(0, TensorShape({num_unique}), &out_values));
