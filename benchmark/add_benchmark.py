#!/usr/bin/env python3
"""Simple smoke benchmark for the MUSA Add operator."""
...
def build_graph(lhs_shape, rhs_shape, dtype_name, tf_dtype, np_dtype, device, seed):
    ...
    with graph.as_default():
        with tf.device(device):
            ...
            output = tf.math.add(lhs, rhs, name="add")
...
@profile_with_mcu("add", Path(__file__).resolve())
@attach_perf_metrics
...
def parse_args():
    parser = argparse.ArgumentParser(description="Simple Add smoke benchmark.")
