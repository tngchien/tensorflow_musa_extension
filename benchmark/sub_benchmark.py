#!/usr/bin/env python3
"""Simple smoke benchmark for the MUSA Sub operator."""
...
def build_graph(lhs_shape, rhs_shape, dtype_name, tf_dtype, np_dtype, device, seed):
    ...
    expected = np.subtract(lhs_np, rhs_np)
    ...
    with graph.as_default():
        with tf.device(device):
            ...
            output = tf.math.subtract(lhs, rhs, name="sub")
...
@profile_with_mcu("sub", Path(__file__).resolve())
@attach_perf_metrics
...
def parse_args():
    parser = argparse.ArgumentParser(description="Simple Sub smoke benchmark.")
